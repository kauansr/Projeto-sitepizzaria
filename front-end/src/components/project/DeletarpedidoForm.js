...
function DeletarpedidoForm() {
    ...
    const handleSubmit = data => {
        ...
        axios.delete(`http://localhost:5000/pedidos/${id}`, { headers: auth }).then((res) => { if (res.data.info_message) { navigate('/produtos') } else { navigate(`/pedidos`) } }).catch(err => console.log(err))
